...
footX = mcm.get_footX(); --why is this different?
...
-- This is the current arm pos
bLArmOR0,bRArmOR0={},{};
bLArmOR0[1],bLArmOR0[2],bLArmOR0[3]=qLArm0[1],qLArm0[2],qLArm0[3];
bRArmOR0[1],bRArmOR0[2],bRArmOR0[3]=qRArm0[1],qRArm0[2],qRArm0[3];

-- this was the last arm pos
bLArmOR,bRArmOR={},{};
bLArmOR[1],bLArmOR[2],bLArmOR[3]=qLArm0[1],qLArm0[2],qLArm0[3];
bRArmOR[1],bRArmOR[2],bRArmOR[3]=qRArm0[1],qRArm0[2],qRArm0[3];
...
function entry()
  ...
  --upper_body_override_on(); --- we want the upper body stuff. DREW
  walkKickRequest = 0;
  stepkick_ready = false;
  stepKickRequest=0;
  velCurrent = {0,0,0};
...
--    print("xdot1:",dx1,dy1);
  ...
  motion_legs(qLegs);
  motion_arms();
	update_arm_balance(); -- balance DREW
...
--Why is armShiftX set to armImuaramY? What does it do?
...
--Call this function every cycle it is a Mahony Filter and from my experience it is ideal for
--this task also this code avoids the common issue of Euler Angles uses quaternions instead
-- which solves the issue of gimbal lock.
--The following are global constants necessary for operation
sampleFreq=512;
twoKpDef=(2 * 0.5);
twoKiDef=(2 * 0);
---------------------------------------------------------------------------------------------------
--Global Variable definitions
twoKp = twoKpDef;	--Proportional Gain					
...
integralFBx = 0;
integralFBy = 0; 
integralFBz = 0;	-- integral error terms scaled by Ki
--
--
--
--
...
function MahonyAHRSupdateIMU(gx,gy,gz,ax,ay,az)
	...
	if(not((ax == 0.0) and (ay == 0.0) and (az == 0.0))) then
		...
		if(twoKi > 0.0) then
			integralFBx += twoKi * halfex * (1.0 / sampleFreq);
...
function update_arm_balance()
 -- if (flag_walk == 100) then
	--  flag_walk = 0;
	--	walk.stop();
	--end
	--if(flag_walk == 1500) then
	--  flag_walk = 0;
	--	walk.start();
	--end
  --flag_walk= flag_walk+1;
 -- print("flag amt: "..flag_walk);
...
--print("gyro data: " .. gyro_roll .. "  pitch: " .. gyro_pitch);

  
	-- past arm data
	bLArmOR[1],bLArmOR[2],bLArmOR[3]= bLArmOR0[1],bLArmOR0[2],bLArmOR0[3];
  bRArmOR[1],bRArmOR[2],bRArmOR[3]=bRArmOR0[1],bRArmOR0[2],bRArmOR0[3];


-- in radians current arm data
  bLArmOR0 = Body.get_larm_position();
	bRArmOR0 = Body.get_rarm_position();  
		 

--	print("left arm data " .. bLArmOR0[1] .. ", " .. bLArmOR0[2] .. ", " .. bLArmOR0[3]);


	-- creating the new data structures for the new arm position data here:
  qLArmActual = {}
	qRArmActual = {}
...
-- balance code here
-- Note: as the units for the arms here are in rad!
    theta_p = gyro_pitch
    theta_r = gyro_roll

    RSPc = bRArmOR0[1] -- this should be the commanded value for the arm
    RSRc = bRArmOR0[2] -- this should be the commanded value for the arm
    REBc = bRArmOR0[3] -- this should be the commanded value for the arm
    LSPc = bLArmOR0[1] -- this should be the commanded value for the arm
    LSRc = bLArmOR0[2] -- this should be the commanded value for the arm
    LEBc = bLArmOR0[3] -- this should be the commanded value for the arm
    ...
    -- pitch START
    -- ---------------------------
    kp_pitch = 1.5 -- change me (sign might also need to be changed)
	  RSP = kp_pitch * theta_p
	  RSPf = RSP + RSPo
    --print("RSPF: " .. RSPf .. " theta_p: " .. theta_p);
    -- Mind sat (pitch)
	  if RSPf > RSPm then
	    RSPf = RSPm
	  end
    ...
    -- roll START
    -- ---------------------------
    kp_roll = 0.9 -- change me (sign might also need to be changed)
		--print ("Theta_r: " .. theta_r);
		RSR = kp_roll * theta_r
		LSR = RSR
	   RSRf = RSR + RSRo
	   LSRf = LSR + LSRo
	--	print("LSRF: " .. LSRf .. " RSRf: " .. RSRf);
			  -- Mind sat (roll)
	  if RSRf > RSRo then
	    RSRf = RSRo
	  end
	...
	--print("Check L: " .. LSRf .. " " .. RSRf);
    -- ---------------------------
    -- roll END
    -- ---------------------------

    --Test with timer - LAURA
	--	if(armTimer < 0) then
			  --Remove actuator velocity limits
     -- for i = 3,5 do
			--	        Body.set_actuator_velocity(0.9, i);
    --  end
	--	armTimer = 0;
	--	elseif (armTimer < 500) then
	--		RSPf = 1.57;
	--		LSPf = 0;
	--		RSRf = 0;
	--		LSRf = 0;
	--	armTimer = armTimer + 1;
	--	elseif (armTimer < 1000)then
	--			RSPf = 1.57;
	--			LSPf = 1.57;
	--			RSRf = 0;
	--			LSRf = 0;
...
function stance_reset() --standup/sitdown/falldown handling
  ...
  print("uLeft values: " .. uLeft[1] .. " " .. uLeft[2] .. " " .. uLeft[3]);
	print("uRight values: " .. uRight[1] .. " " .. uRight[2] .. " " .. uRight[3]);
	print("uTorso values: " .. uTorso[1] .. " " .. uTorso[2] .. " " .. uTorso[3]);
