...
function update()
  ...
  --[[
  	Some parameter setting used in this function:
  	active = true;
	started = false;
	iStep0 = -1;
	iStep = 0;
...
--    print("xdot1:",dx1,dy1);
  ...
  xFoot, zFoot = foot_phase(ph);  --  ph = (t-tLastStep)/tStep
...
----------------------------------------------------------
--           Walking and doing something else
----------------------------------------------------------

-- david said all these camel style function are written by Drew and himself
...
function motion_legs(qLegs,gyro_off)
  ...
  gyro_pitch = gyro_pitch0*math.cos(yawAngle)
	...
	-- signature: procFunc(a,deadband,maxvalue)
	-- armImuParamY[2]:gain
	-- armImuParamY[3]:deadband
	-- armImuParamY[4]:max
...
-- balance code here
-- Note: as the units for the arms were in deg, i assuem all units are in deg
    theta_p = gyro_pitch
    theta_r = gyro_roll
...
function stance_reset() --standup/sitdown/falldown handling
  if start_from_step then
    ...
    print("uTorso : " .. uTorso[1] .. " " .. uTorso[2] .. " " .. uTorso[3]);
		print("supportX: " .. supportX .. " footY: " .. footY);
	...
	print("uLeft value: " .. uLeft[1] .. " " .. uLeft[2] .. " " .. uLeft[3]);
	print("uRight value: " .. uRight[1] .. " " .. uRight[2] .. " " .. uRight[3]);
	print("uTorso value: " .. uTorso[1] .. " " .. uTorso[2] .. " " .. uTorso[3]);
...
-- haven't been used in anywhere
end

function zero_velocity()
-- dummy function for NSL kick, depreciated
-- do nothing
-- only place called is PunchKick.lua
end
