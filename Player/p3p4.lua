...
circleBallStart = function(hfa)                   
end	
circleBallGo = function(hfa)
	local ySpd = walkSpeed * 2
	local xSpd = 0
	local aSpd = turnSpeed * -2


		darwin.setVelocity(xSpd, ySpd, aSpd)

end
circleBallStop = function (hfa)
end
...
circleBall = makeBehavior("circleBall", circleBallStart, circleBallStop, circleBallGo);
...
myMachine = makeHFA("myMachine", makeTransition(
	{
		[start] = circleBall, --first thing we do: walk forward
		[localize] = function()
